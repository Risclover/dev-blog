...
Not shown in the above code are the `NavBar` and `UserProfile` components, to which all of our state variables are passed, since they'll be needing them as well.
beyond using the state variables with the Chat component, they are also passed to the `NavBar` and `UserProfile` components, since we'll be needing them later.
