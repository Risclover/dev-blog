...
Our chat thread model needs properties for the thread's messages and the thread's users. The relationship between messages and chat threads is one-to-many (one thread has many messages, many messages belong to one thread), and the relationship between users and chat threads is 
