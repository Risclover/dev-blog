...
With our models designed and written, we can move on to our endpoints. We can refer to the same list we referenced in the previous section to help us figure out what endpoints we need:


Creating a new chat thread by clicking the "Create Chat" button.
Sending messages through the input box.
Clicking on a chat navigation button to open the respective thread.
"Deleting" a message by updating its content.
