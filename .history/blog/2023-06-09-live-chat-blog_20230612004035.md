...
Next, we create a new file, `socket.py`, which lives in the general `app/` folder instead of `api/`. Here, we handle the remaining backend code needed to implement WebSockets.
...
Now we can start writing our endpoints. We begin by creating an empty list that we'll call `chat_users`:

```python
chat_users = []
```

This will be a list of the users actively logged in to the live chat servers.
