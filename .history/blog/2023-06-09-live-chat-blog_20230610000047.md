...
Lastly, the thread's users property (which we'll call `chat_thread_users`) needs to reference the many-to-many relationship between threads and users. Similarly to the `messages` property, we need to use `db.relationship()`
