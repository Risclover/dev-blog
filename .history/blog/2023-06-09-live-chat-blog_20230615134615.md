...
However, according to a thorough write-up published by Booking.com, using snippets starting with `-apple-system` may cause some browsers to view the leading font as a vendor prefix that will be ignored.

Chrome and Safari recently shipped "system-ui", which is a generic font family that can be used in place of `-apple-system` and `BlinkMacSystemFont`.
:::
