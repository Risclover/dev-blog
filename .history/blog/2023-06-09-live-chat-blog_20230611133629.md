...
The `chat_thread_users` field uses the `secondary` argument to associate the `user_chat_threads` table, which creates a many-to-many relationship between users and chat threads.
