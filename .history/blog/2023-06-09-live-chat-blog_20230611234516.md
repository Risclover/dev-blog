...
For example, after setting `request.get_json()` to a variable called `data`, when we write the code `data["receiverId"]`, `receiverId` is coming from incoming JSON, set by us in the front end.
