...
Since chat threads have multiple (well, _two_) users and users can have many chat threads, the relationship is many-to-many. To achieve a many-to-many relationship between two tables, we'll need to create an association table with two foreign keys - one for `users` and one for `chat_threads`. In addition, when we create our `ChatThread` model, we need to remember to assign the `secondary` attribute to the property representing the thread's users.
