...
As a quick side note, some of the code snippets in this blog post have an import statement at the top that says `from .db import db`. To clear up any potential confusion, this is referring to the `db.py` file, which holds the following code:that the import statement at the top of some of the code snippets in this blog post, `from .db import db`, is referring to the following code, which lives in a separate file called `db.py`:
