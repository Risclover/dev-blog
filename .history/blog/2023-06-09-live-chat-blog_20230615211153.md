...
Since we know that we're going to need two access points that are in two completely different components, we need to create the state variable in a place that can reach both components. For Ribbit, that would be all the way back in `App.js`, in which we add this code:
...
Moving on to creating the actual button, let's take another look at it.
...
Reddit has given it this icon - a chat bubble with 3 dots inside - so let's try to find that icon or something similar.

:::note Finding Icons
When it comes to finding an icon, typically don't like to deviate from [react-icons](https://react-icons.github.io/react-icons/), because doing so usually requires downloading the image instead of importing it from a library and using it in the code, like you do with the icons from react-icons.
...
Included in the above code is the code that handles the button's tooltip.

1. We give our "button" (a simple `div` element instead of an actual `button` element) three events, two of which are `onMouseEnter` and `onMouseLeave`:
...
2. Inside of the button is the tooltip itself:
   ...
   So, when the `showTooltip` variable is `true`, the tooltip, which says "Chat", appears.

This is the tooltip that appears when the mouse hovers over the button, as shown in the screenshot of the chat button above.
