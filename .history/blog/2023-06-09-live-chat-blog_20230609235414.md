...
The thread's messages (which we'll call `messages`) needs to reference the one-to-many relationship between threads and messages, so we'll use `db.relationship()` instead of `db.Column()` with `back_populates='chat_message_thread'`.
