...
Locating an icon that looked like the one on Reddit was simple using my usual source, [React-Icons](https://react-icons.github.io/react-icons/). Eventually, I went onto [The Noun Project](https://thenounproject.com) and found one that looked really close, which you can find [here](https://thenounproject.com/icon/create-chat-5697272/).

:::note
Just as an FYI, I typically don't like to deviate from finding icons on [React-Icons](https://react-icons.github.io/react-icons/), because doing so usually requires downloading the image instead of importing it from a library and using it in the code, like you do with React-Icons icons.
...
After placing this icon in a button and putting it in my `NavBar` component, I added a click event to the button.
...
return (
  <button className="navbar-btn-chat" onClick={handleOpenChat}>
    <img src="../create-chat-icon.png" alt="Chat" />
  </button>
);
```
