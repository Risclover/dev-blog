...
For example, after setting `request.get_json()` to a variable called `data`, when we write the code `data.get("receiverId")`, `data["receiverId"]` is literally the receiver's id, which we set to the `receiverId` variable in the frontend before converting it to JSON.

* `receiver = User.query.get(data["receiverId"])` - We find the receiver by their user id and set it to the `receiver` variable
...
We now move on to the endpoints for chat messages. Just like with chat threads, we need to be able to create a message with `POST`:
