...
I used [react-icons](https://react-icons.github.io/react-icons) to find the icon I needed (re: chat bubble with 3 dots inside), went into my `NavBar` component, and created the button.
...
:::note Finding Icons
When it comes to finding an icon, typically don't like to deviate from finding icons on [React-Icons](https://react-icons.github.io/react-icons/), because doing so usually requires downloading the image instead of importing it from a library and using it in the code, like you do with React-Icons icons.
