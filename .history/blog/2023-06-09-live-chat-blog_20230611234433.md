...
We are also utilizing the `request` object - specifically, the `get_json()` method, which parses the incoming JSON request data and returns it so we may reference it. For example, after setting it to a variable called `data`, when we write the code `data["receiverId"]`, `receiverId` is coming from the incoming JSON request data
