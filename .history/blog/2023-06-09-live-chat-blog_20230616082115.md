...
And that's all there is to this button. The most important parts are the `setOpenChat` and `setSelectedChat` props, which handle the chat window's visibility and the thread 
