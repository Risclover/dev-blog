...
Since chat threads have multiple (well, _two_) users and users can have many chat threads, the relationship is many-to-many. This means that we also need an association table joining users with chat threads that we'll call `user_chat_threads`.
