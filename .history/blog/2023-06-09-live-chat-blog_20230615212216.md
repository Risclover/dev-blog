...
Since we know that we're going to need two access points that are in two completely different components, we need to create the state variables in a place that can reach both components. For Ribbit, that would be all the way back in `App.js`, in which we add this code:
