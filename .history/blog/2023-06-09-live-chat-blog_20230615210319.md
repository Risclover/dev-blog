...
Moving on to creating the actual button, let's take another look at it. Reddit has given it this icon - a chat bubble with 3 dots inside - so let's try to find that icon or something similar.
...
In this case, I used [react-icons](https://react-icons.github.io/react-icons) library to find the icon I needed (re: chat bubble with 3 dots inside), went into my `NavBar` component, and created the button.

After placing this icon in a button and putting it in my `NavBar` component, I added a click event to the button. Here is the relevant code from my `NavBar` component:
