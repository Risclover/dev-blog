...
The chat window is divided into two main parts - the left side and the right side - so we decided to structure the folders like that by creating a `ChatLeft` folder and a `ChatRight/` folder.
