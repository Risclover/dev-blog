...
Note that not shown are beyond using the state variables with the Chat component, they are also passed to the `NavBar` and `UserProfile` components, since we'll be needing them later.
