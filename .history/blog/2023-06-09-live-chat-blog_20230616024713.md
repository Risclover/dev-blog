...
Next, we create the `NavBar` component, making sure to import the `setOpenChat`, `openChat`, and `setSelectedChat` props. We use the `useDispatch()` hook (giving it the `dispatch` variable), followed by setting the variable `chatThreads` to the chat threads in the redux state, which we 
