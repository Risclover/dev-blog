...
As a quick side note, some of the code snippets in this blog post have that the import statement at the top of some of the code snippets in this blog post, `from .db import db`, is referring to the following code, which lives in a separate file called `db.py`:
