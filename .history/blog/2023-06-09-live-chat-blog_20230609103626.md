...
* There are tons of details in the messages area:
  ...
  * Messages are *usually* labeled with the sending user's username and image, as well as the time the message was sent.
    * When one user sends multiple messages in a row:
      * Messages sent within a minute of the message before it are grouped together with that message. Except for the first message in a group, grouped messages don't have the user's information or the time sent on display
