...
* `messages` - One-to-many relationship with the  `ChatMessage` model to represent the messages in the thread
* `chat_thread_users` - Many-to-many relationship with the `User` model to associate the users participating in the chat thread. Note the `secondary` argument, which associates the `user_chat_threads` table.
